...
var Databases = map[string]Database{
	"postgres": {
		...
		CreatePolicy:             `INSERT INTO ladon_policy(id, description, effect, conditions) SELECT $1, $2, $3, $4 WHERE NOT EXISTS (SELECT 1 FROM ladon_policy WHERE id = $1)`,
		CreatePolicyActions:      `INSERT INTO ladon_action (id, template, compiled, has_regex) SELECT $1, $2, $3, $4 WHERE NOT EXISTS (SELECT 1 FROM ladon_action WHERE id = $1)`,
		CreatePolicyActionsRel:   `INSERT INTO ladon_policy_action_rel (policy, action) SELECT $1, $2 WHERE NOT EXISTS (SELECT 1 FROM ladon_policy_action_rel WHERE policy = $1 AND action = $2)`,
		CreatePolicyResources:    `INSERT INTO ladon_resource (id, template, compiled, has_regex) SELECT $1, $2, $3, $4 WHERE NOT EXISTS (SELECT 1 FROM ladon_resource WHERE id = $1)`,
		CreatePolicyResourcesRel: `INSERT INTO ladon_policy_resource_rel (policy, resource) SELECT $1, $2 WHERE NOT EXISTS (SELECT 1 FROM ladon_policy_resource_rel WHERE policy = $1 AND resource = $2)`,
		CreatePolicySubjects:     `INSERT INTO ladon_subject (id, template, compiled, has_regex) SELECT $1, $2, $3, $4 WHERE NOT EXISTS (SELECT 1 FROM ladon_subject WHERE id = $1)`,
		CreatePolicySubjectsRel:  `INSERT INTO ladon_policy_subject_rel (policy, subject) SELECT $1, $2 WHERE NOT EXISTS (SELECT 1 FROM ladon_policy_subject_rel WHERE policy = $1 AND subject = $2)`,
