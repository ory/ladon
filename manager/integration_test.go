...
func ConnectToMySQL() *sqlx.DB {
	...
	if err = pool.Retry(func() error {
		...
		db, err = sqlx.Open("mysql", fmt.Sprintf(
			"root:secret@(127.0.0.1:%s)/mysql?parseTime=true",
...
func ConnectToPostgres(database string) *sqlx.DB {
	...
	if err = pool.Retry(func() error {
		var err error
		db, err = sqlx.Open("postgres", fmt.Sprintf("postgres://postgres:secret@127.0.0.1:%s/%s?sslmode=disable", resource.GetPort("5432/tcp"), database))
		if err != nil {
			return err
		}
		return db.Ping()
...
func ConnectToRethinkDB(database string, tables ...string) *r.Session {
	...
	if err := pool.Retry(func() error {
		if session, err = r.Connect(r.ConnectOpts{Address: fmt.Sprintf("127.0.0.1:%s", resource.GetPort("28015/tcp")), Database: database}); err != nil {
			return err
		}
